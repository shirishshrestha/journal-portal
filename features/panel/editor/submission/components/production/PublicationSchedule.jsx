...
import { Calendar, BookOpen, CheckCircle2, XCircle, Loader2, Edit, Globe } from 'lucide-react';
...
export function PublicationSchedule({ submission, submissionId }) {
  ...
  const { data: schedules = [], isLoading, error } = usePublicationSchedules(submissionId);
    ...
    <>
        ...
        <CardContent>
          {!schedule ? (
            <div className="flex flex-col items-center justify-center py-12 text-center border-2 border-dashed rounded-lg">
              ...
              <h3 className="text-lg font-semibold mb-2">No Publication Scheduled</h3>
              <p className="text-sm text-muted-foreground max-w-md mb-4">
                Schedule this article for publication by providing volume, issue, and publication
                date information.
              </p>
            ...
            <div className="space-y-6">
              ...
              {schedule.doi && (
                <div>
                  ...
                  <p className="font-medium font-mono text-sm break-all">{schedule.doi}</p>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-medium">
                    {format(new Date(schedule.scheduled_date), "MMM d, yyyy 'at' h:mm a")}
                  </p>
                </div>
                {schedule.published_date && (
                  <div>
                    ...
                    <p className="font-medium">
                      {format(new Date(schedule.published_date), "MMM d, yyyy 'at' h:mm a")}
                    </p>
                  </div>
                )}
              </div>
              ...
              {schedule.scheduled_by && (
                <div>
                  <p className="text-sm text-muted-foreground mb-1">Scheduled By</p>
                  <p className="font-medium">
                    {schedule.scheduled_by.user?.first_name} {schedule.scheduled_by.user?.last_name}
                  </p>
                </div>
              )}
      ...
      <Dialog open={isScheduleDialogOpen} onOpenChange={setIsScheduleDialogOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>
              {isEditMode ? 'Edit Publication Schedule' : 'Schedule Publication'}
            </DialogTitle>
            <DialogDescription>
              Provide publication metadata including volume, issue, and publication date
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="scheduled_date">
                Scheduled Publication Date <span className="text-destructive">*</span>
              </Label>
              <Input
                id="scheduled_date"
                type="datetime-local"
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setScheduleData((prev) => ({ ...prev, doi: e.target.value }))}
