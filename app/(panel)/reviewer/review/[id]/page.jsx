  ...
  if (error || !assignment) {
    return (
      <ErrorCard
        ...
        description={
          error?.message || 'Unable to load review assignment details. Please try again.'
        }
        onRetry={refetch}
        onBack={() => router.back()}
      />
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col-reverse sm:flex-row items-start sm:items-center justify-between">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground">Review manuscript and provide feedback</p>
        </div>
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="sm" onClick={() => router.push(`/reviewer/assignments`)}>
        ...
        <CardContent className="space-y-4">
          ...
          {assignment.status === 'DECLINED' && assignment.decline_reason && (
            <>
              <Separator />
              <div>
                <h3 className="font-semibold mb-2 text-destructive">Decline Reason</h3>
                <div className="p-3 bg-destructive/10 dark:bg-destructive/20 border border-destructive/20 dark:border-destructive/30 rounded-lg">
      ...
      {submission && (
        <Tabs defaultValue="details">
          ...
          <TabsContent value="details" className="space-y-4 mt-4">
            <SubmissionDetailsTab submission={submission} isPending={isLoading} />
          </TabsContent>
