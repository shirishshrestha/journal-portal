...
export default function PendingAssignmentsPage() {
  ...
  const { data: assignmentsData, isPending, error, refetch } = useGetPendingAssignments({ params });
  ...
  const handleAccept = (assignment) => {
    acceptMutation.mutate(assignment.id, {
      ...
      onError: (error) => {
        toast.error(`Failed to accept: ${error.response?.data?.error || error.message}`);
      },
    });
  };
    ...
    return (
      <div className="mt-6">
        <ErrorCard
          ...
          description={
            error?.message || 'Unable to load your pending review assignments. Please try again.'
          }
          onRetry={refetch}
        />
      </div>
