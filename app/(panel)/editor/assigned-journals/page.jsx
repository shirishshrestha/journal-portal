  ...
  const columns = [
    {
      ...
      render: (row) => (
        <div className="max-w-md">
          ...
          {row.abbreviation && <p className="text-xs text-muted-foreground">{row.abbreviation}</p>}
        </div>
      ),
    },
    {
      ...
      header: 'My Role',
      render: (row) => {
        const myStaffEntry = row.staff_members?.find((member) => member.email === userEmail);
    ...
    },
    {
      ...
      render: (row) => (
        <Button
          variant="ghost"
          size="sm"
          onClick={() => router.push(`/editor/assigned-journals/${row.id}/submissions`)}
        >
  ...
  if (journalsError) {
    return (
      <ErrorCard
        ...
        description={journalsError?.message || 'Unable to load journals. Please try again later.'}
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          <h1 className="text-3xl font-semibold text-foreground">Assigned Journals</h1>
          <p className="text-muted-foreground">Journals where you are assigned as a staff member</p>
        </div>
      </div>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">Journals you&apos;re assigned to</p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-semibold">
                {journals.reduce((acc, journal) => acc + (journal.submissions_count || 0), 0)}
              </div>
