  ...
  if (submissionError) {
    return (
      <ErrorCard
        ...
        description={submissionError?.message || 'Error loading submission details.'}
        onRetry={() => router.back()}
      />
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          ...
          <h1 className="text-3xl font-semibold text-foreground">Reviews for Submission</h1>
          <p className="text-muted-foreground">{submission?.title || 'Loading...'}</p>
        </div>
        <Button
          variant="outline"
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <p className="font-medium">{submission?.submission_number || 'N/A'}</p>
            </div>
            <div>
      ...
      {reviews.length === 0 ? (
        ...
        <div className="space-y-4">
          <h2 className="text-xl font-semibold">Submitted Reviews ({reviews.length})</h2>
          {reviews.map((review, index) => (
            <div key={review.id}>
              <ReviewSummaryCard
                reviews={[review]}
                showViewFullReview={true}
                onViewFullReview={(reviewId) => router.push(`/editor/reviews/${reviewId}`)}
                title={`Review ${index + 1}${
                  review.review_round > 1 ? ` (Round ${review.review_round})` : ''
                }`}
              />
            </div>
          ))}
